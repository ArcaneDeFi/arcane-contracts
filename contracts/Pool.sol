...
contract Pool is IPool, Ownable {
    ...
    onlyOwner
  {
    ...
    _rewardToken.safeTransferFrom(_msgSender(), address(this), totalSupply_);
  }
  ...
  function addDepositAmount(
    ...
    for (uint256 index = 0; index < addrArr_.length; index++) {
      ...
      require(convertAmount <= remainingAllocation, "Not enough allocation");
  ...
  function deposit(uint256 amount_) external virtual override {
    ...
    _depositToken.safeTransferFrom(_msgSender(), address(this), transferAmount);
  ...
  function _setVesting(
    ...
    if (VestingType.LINEAR_VESTING == _vestingType) {
      ...
      for (uint256 i = 0; i < _intervals.length; i++) {
        ...
        require(
          percent > lastUnlockingPart && percent <= _MAX_INITIAL_PERCENTAGE,
  ...
  /// @return Unlocked amount of reward tokens
  function _calculateLinearUnlock(VestingInfo memory info, uint256 tokenAmount)
    internal
    view
    virtual
    returns (uint256)
  {
    if (block.timestamp > _endDate + info.cliffDuration) {
      ...
      uint256 passePeriod = Math.min(
        (block.timestamp - _endDate - info.cliffDuration) / info.periodDuration,
